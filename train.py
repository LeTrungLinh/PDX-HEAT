...
wandb.login(key="1f751630c11b529e404f0a5b8bbf9f8f4136a3d2")
...
def train_one_epoch(image_size, backbone, corner_model, edge_model, corner_criterion, edge_criterion, data_loader,
    ...
    for step, data in loop:
        ...
        loss = awl(s1_losses , s2_losses_hb , s2_losses_rel , corner_loss*args.lambda_corner)
        ...
        optimizer.step()
        wandb.log({'Others/learning_rate': optimizer.param_groups[0]["lr"]})
        # loop.set_description(f"Epoch [{epoch}/{args.epochs}]")
        loop.set_description("Epoch [{}/{}]".format(epoch, args.epochs))
        loop.set_postfix(loss = loss_value, corner_recall = corner_recall.item())
    wandb.log({'Train/loss_e_s1': s1_losses, 'Train/loss_e_s2_hb': s2_losses_hb, 'Train/loss_e_s2_rel': s2_losses_rel,
            'Train/edge_acc_s1': s1_acc, 'Train/edge_acc_s2_hb': s2_acc_hb, 'Train/edge_acc_s2_rel': s2_acc_rel,
            'Train/loss_c_s1': corner_loss, 'Train/corner_recall': corner_recall, 'Train/total_loss' : loss})
...
def evaluate(image_size, backbone, corner_model, edge_model, corner_criterion, edge_criterion, data_loader, epoch,
    ...
    loop = tqdm(enumerate(data_loader), total=len(data_loader), colour='BLUE')
    for step, data in loop:
        ...
        # wandb.log({'Validate/loss_e_s1': s1_losses, 'Validate/loss_e_s2_hb': s2_losses_hb, 'Validate/loss_e_s2_rel': s2_losses_rel,
        #             'Validate/edge_acc_s1': s1_acc, 'Validate/edge_acc_s2_hb': s2_acc_hb, 'Validate/edge_acc_s2_rel': s2_acc_rel,
        #             'Validate/loss_c_s1': corner_loss, 'Validate/corner_recall': corner_recall, 'Validate/total_loss': loss})
        
        loop.set_description(f"Validate")
        loop.set_postfix(loss = loss_value, corner_recall = corner_recall.item())
...
def main():
    ...
    project="2d_heatmodel",
    name = "transfer-2000image-9_6_2023-newloss_lr2e-4",
    # track hyperparameters and run metadata
    config={
    "learning_rate": args.lr,
    "architecture": "2d_heatmodel + ResNet",
    # "dataset": "Floorplan 500",
    "dataset": "transfer learning + own 500 data",
    'lr drop' : args.lr_drop,
    'max corner nums': args.max_corner_num,
    'lambda corner' : args.lambda_corner,
    "epochs": args.epochs,
    ...
    if args.exp_dataset == 'outdoor':
        # data_path = './data_heat_model/outdoor/cities_dataset'
        data_path = './data_train'

        det_path = './data_heat_model/outdoor/det_final'
    ...
    # optimizer = SophiaG([
    #                 {'params': all_params},
    #                 {'params': awl.parameters(), 'weight_decay': args.weight_decay}
    #             ], lr=args.lr)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        wandb.log({'Others/epoches':epoch, 'Others/val_acc': val_acc})
