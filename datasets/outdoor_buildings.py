...
class OutdoorBuildingDataset(CornersDataset):
    ...
    def __getitem__(self, idx):
        ...
        data_name_annot = data_name.replace('rgb','annot')
        annot_path = data_name_annot.replace('.jpg', '.npy').replace('.JPG', '.npy').replace('.png', '.npy').replace('.PNG', '.npy')
        annot_org = np.load(annot_path, allow_pickle=True, encoding='latin1').tolist()
        ...
        annot = dict()
        # scale the coordinates in the annotations
        for point, connections in annot_org.items():
            x, y = point
            scaled_point = (float(x * 256 / rgb.shape[1]), int(y * 256 / rgb.shape[0]))
            scaled_connections = []
            for connection in connections:
                
                scaled_connection = [float(connection[0] * 256 / rgb.shape[1]), int(connection[1] * 256 / rgb.shape[0])]
                scaled_connections.append(scaled_connection)
            
            annot[scaled_point] = scaled_connections
        ...
        if self.rand_aug:
            image, annot, corner_mapping, det_corners = self.random_aug_annot(rgb, annot, det_corners=None)
        else:
            image = rgb
        rec_mat = None
