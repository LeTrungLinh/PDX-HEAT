...
class CornersDataset(Dataset):
    ...
    def get_corner_labels(self, corners):
        ...
        if np.any(xint < 0) or np.any(xint >= self.image_size):
            out_of_bounds_indices = np.where((xint < 0) | (xint >= self.image_size))[0]
            xint[out_of_bounds_indices] = xint[out_of_bounds_indices] -1
            # print(Fore.RED + "the point out of images.............")
        if np.any(yint < 0) or np.any(yint >= self.image_size):
            raise IndexError(f"yint is out of bounds with value is {yint}")
        ...
        # mask = (corners[:, 0] >= 0) & (corners[:, 0] < labels.shape[1]) & (corners[:, 1] >= 0) & (corners[:, 1] < labels.shape[0])
        # corners = corners[mask]
        # labels[corners[:, 1], corners[:, 0]] = 1

        gauss_labels = gaussian_filter(labels, sigma=2)
        gauss_labels = gauss_labels / gauss_labels.max()
        return labels, gauss_labels

    def resize_data(self, image, annot):
        ...
        for c, connections in annot.items():
            ...
            new_connections = [tuple(np.array(other_c) * r) for other_c in connections]
